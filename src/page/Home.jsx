...
import { Badge, Col, Container, Form, Nav, Navbar, NavDropdown } from "react-bootstrap";
...
const Home = () => {
    ...
    <React.Fragment>
        ...
        <Container>
          <Navbar.Brand href="#"><strong>Slebew</strong>Mart</Navbar.Brand>
          ...
          <Navbar.Collapse id="navbarScroll">
            ...
            >
              ...
              <select style={{
                ...
                onChange={e => {handleClick(e.target.value)}} title="Category" id="navbarScrollingDropdown">
                <option style={{background: "white", color: "black"}} value="Semua Menu">
                  Semua Menu
                </option>
                <NavDropdown.Divider />
                <option style={{background: "white", color: "black"}} value="Makanan">
                  Makanan
                </option>
                <NavDropdown.Divider />
                <option style={{background: "white", color: "black"}} value="Minuman">
                  Minuman
                </option>
                <NavDropdown.Divider />
                <option style={{background: "white", color: "black"}} value="Snack">
                  Snack
                </option>
              </select>
        ...
        <h5 className="tags" >Tags :</h5>  
            ...
            {tags.map((item, index) => (
                <button key={index} value={tags} className="button-tags"><FontAwesomeIcon icon={faTag} />{item.name}</button>
            ))}
            </div>
            ...
            {/*---------------------------------------------------------------*/}
                ...
                <br />
                {products.length === 0 && <h6 className="none-menu">Maaf menu tidak ditemukan</h6>}
                {products.map((item, index) => (
                <Col md={3} xs={6} className="mb-1 mt-4">
                <Card key={index} className="shadow" style={{border: "5px solid white"}}>
                <Card.Img style={{cursor:"pointer"}} variant="top" src={"http://localhost:8000/images/products/" + item.image_url} />
                <Card.Body>
                    ...
                    {item.tags.map((value, index) => (
                        <button key={index} className="tag-item"><FontAwesomeIcon icon={faTag}/>{value.name}</button>
                    ))}
                    <button style={{
                        background: "red",
                        borderRadius: "5px",
            ...
            <Pagination className="page">
                ...
                <Pagination.Item className="page3" active>{1}</Pagination.Item>
